...
impl TokenWorkload {
    ...
    async fn execute_inner(&self) -> Result<LatencySample> {
        // 1. Build MoveCall PTB: token::mint(1000)
        let tx_bytes = self
            .ptb
            .build_move_call(
                &self.sender,
...
impl Workload for TokenWorkload {
    fn name(&self) -> &str {
        if let Some(ref name) = self.name_override {
            return name;
        }
        match self.mode {
            SubmitMode::DirectValidator => "direct-mint",
            SubmitMode::GrpcFullnode => "grpc-mint",
            SubmitMode::JsonRpc => "vanilla-mint",
            SubmitMode::CdnFast => "cdn-fast-mint",
            SubmitMode::CdnPerServer => "cdn-per-server-mint",
        }
    }

    fn description(&self) -> &str {
        "BenchToken mint (owned object, fast path)"
    }
