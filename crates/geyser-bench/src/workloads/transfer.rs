...
/// SUI-to-self transfer workload.
/// Owned objects → fast path (no consensus).
...
impl TransferWorkload {
    ...
    /// Look up the label for a URL, falling back to the URL itself.
    ...
    async fn execute_inner(&self) -> Result<LatencySample> {
        // 1. Build PTB: transfer 1 MIST to self
        let tx_bytes = self
            .ptb
            .build_transfer_sui(&self.sender, &self.sender, 1, 10_000_000)
            .await?;
        ...
        match self.mode {
            ...
            SubmitMode::CdnFast | SubmitMode::CdnPerServer => {
                ...
                // Use detailed race to get per-server timing
                let race = self
                    .ptb
...
impl Workload for TransferWorkload {
    fn name(&self) -> &str {
        ...
        match self.mode {
            SubmitMode::DirectValidator => "direct-transfer",
            SubmitMode::GrpcFullnode => "grpc-transfer",
            SubmitMode::JsonRpc => "vanilla-transfer",
            SubmitMode::CdnFast => "cdn-fast-transfer",
            SubmitMode::CdnPerServer => "cdn-per-server-transfer",
        }
    }

    fn description(&self) -> &str {
        "SUI self-transfer (owned object, fast path)"
    }
